...
describe('FilmsController', () => {
  ...
  describe('findAllFilms', () => {
    ...
    it('должен пробрасывать ошибку от сервиса', async () => {
      mockFilmsService.findAllFilms.mockRejectedValue(
        new NotFoundException('Список фильмов пуст'),
      );

      await expect(controller.findAllFilms()).rejects.toThrow(
        NotFoundException,
      );
      await expect(controller.findAllFilms()).rejects.toThrow(
        'Список фильмов пуст',
  ...
  describe('findFilmSchedule', () => {
    ...
    it('должен пробрасывать ошибку от сервиса', async () => {
      mockFilmsService.findFilmSchedule.mockRejectedValue(
        new NotFoundException('Фильм не найден'),
      );

      await expect(controller.findFilmSchedule(filmId)).rejects.toThrow(
        NotFoundException,
      );
      await expect(controller.findFilmSchedule(filmId)).rejects.toThrow(
        'Фильм не найден',
